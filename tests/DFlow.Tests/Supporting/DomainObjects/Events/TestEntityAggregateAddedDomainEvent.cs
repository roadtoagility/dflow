...
namespace DFlow.Tests.Supporting.DomainObjects.Events
{
    ...
    {
        private TestEntityAggregateAddedDomainEvent(EventStream<EntityTestId> stream)
            : base(stream)
        {
        }
       
        public static TestEntityAggregateAddedDomainEvent For(EventStream<EntityTestId> stream)
        {
            return new TestEntityAggregateAddedDomainEvent(stream);
        }
    }
}